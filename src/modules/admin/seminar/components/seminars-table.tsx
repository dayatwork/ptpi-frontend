...
import { useSuspenseSeminars } from "../seminar.query";
...
import { validFormats, validStatuses } from "../seminar.validation";
...
export function SeminarsTable() {
  ...
  const { setOpen, createSeminar, deleteSeminar, editSeminar, state } =
    useSeminarsAction();
  const { data } = useSuspenseSeminars();
  ...
  const columns: ColumnDef<Item>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) =>
          row.original.price
            ? rupiahFormatter.format(row.original.price)
            : "FREE",
        enableResizing: false,
        size: 80,
        meta: {
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-auto">
              ...
              <DropdownMenuItem asChild>
                <Link to="/admin/seminars/$id" params={{ id: row.original.id }}>
                  <FileText />
                  Details
                </Link>
              </DropdownMenuItem>
