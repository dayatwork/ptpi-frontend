...
export const Route = createFileRoute("/app/events/upcoming")({
  component: RouteComponent,
  loader: ({ context: { queryClient } }) =>
    ensureEventsData(queryClient, "upcoming"),
});

function RouteComponent() {
  const { data: events } = useSuspenseEvents("upcoming");
  return (
        ...
        <Card key={event.id} className="overflow-hidden p-0">
          <div className="aspect-video w-full overflow-hidden relative">
            ...
            <div className="absolute top-2 right-2">
              <Badge>{getDaysLeft(event.startDate)} days left</Badge>
            </div>
          </div>
...
function getDaysLeft(startTime: string | Date): number {
  const now = new Date();
